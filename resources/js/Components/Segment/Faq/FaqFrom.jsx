...
export default function FaqForm({faq, setFaq}){
    ...
    const handleSubmitForm = (e) => {
        ...
        if(!faq){
            post(route('admin.faq.store'), {
                preserveState: true,
                preserveScroll: true,
                onError: () => HSOverlay.open('#faq-form'),
                onSuccess: () => {
                    HSOverlay.close('#faq-form')
                    reset()
                    setFaq(null)
                }
            })
    ...
    return (
        <div id="faq-form"
             ...
             role="dialog" tabIndex="-1" aria-labelledby="faq-form-label">
            <div
                ...
                <div
                    ...
                    <div
                        ...
                        <h3 id="faq-form-label" className="font-bold text-gray-800 text-md dark:text-white">
                            {faq ? 'Update': 'Create'} Form
                        </h3>
                        <button type="button"
                                onClick={(e) => {
                                    reset()
                                ...
                                aria-label="Close" data-hs-overlay="#faq-form">
                    ...
                    <div className="p-4 overflow-y-auto">
                        <form onSubmit={handleSubmitForm}  className={`w-full space-y-2`}>
                            <div className="form-control">
                                <label htmlFor="question" className={`label`}>Question <span className={`text-xs text-red-600`}>*</span></label>
                                <input type="text" id={`question`} value={data.question} onChange={handleTextInput} className={`input`} placeholder={`Enter question`}/>
                            </div>

                            <div className="form-control">
                                <label htmlFor="answer" className={`label`}>Answer <span className={`text-xs text-red-600`}>*</span></label>
                                <input type="text" id={`answer`} value={data.answer} onChange={handleTextInput} className={`input`} placeholder={`Enter answer`}/>
                            </div>

                            <div className="form-control pt-4">
                                <div className="w-full flex gap-x-4">
                                    <Button buttonText={!faq ? 'Create' : 'Update'} isLoading={processing} />
                                    <Button
                                        buttonText={'Cancel'}
                                        type={`button`}
                                        callback={() => HSOverlay.close("#faq-form")}
