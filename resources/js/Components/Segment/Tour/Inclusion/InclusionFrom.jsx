...
export default function InclusionForm({inclusion, setInclusion}){
    ...
    const handleSubmitForm = (e) => {
        ...
        if(!inclusion){
            post(route('admin.tour.inclusion.store'), {
                preserveState: true,
                preserveScroll: true,
                onError: () => HSOverlay.open('#inclusion-form'),
                onSuccess: () => {
                    HSOverlay.close('#inclusion-form')
                    resetData()
                    setInclusion(null)
                }
            })
        }else{
            post(route('admin.tour.inclusion.update'), {
                preserveState: true,
                preserveScroll: true,
                onError: () => HSOverlay.open('#inclusion-form'),
                onSuccess: () => {
    ...
    return (
        <div id="inclusion-form"
             ...
             role="dialog" tabIndex="-1" aria-labelledby="inclusion-form-label">
            <div
                ...
                <div
                    ...
                    <div
                        ...
                        <h3 id="inclusion-form-label" className="font-bold text-gray-800 text-md dark:text-white">
                            {inclusion ? 'Update': 'Create'} Form
                        </h3>
                        <button type="button"
                                onClick={(e) => {
                                    resetData()
                                ...
                                aria-label="Close" data-hs-overlay="#inclusion-form">
                        ...
                        <form onSubmit={handleSubmitForm}  className={`w-full space-y-2`}>
                            ...
                            <div className="form-control pt-4">
                                <div className="w-full flex gap-x-4">
                                    <Button buttonText={!inclusion ? 'Create' : 'Update'} isLoading={processing} />
                                    <Button
                                        buttonText={'Cancel'}
                                        type={`button`}
                                        callback={() => HSOverlay.close("#inclusion-form")}
